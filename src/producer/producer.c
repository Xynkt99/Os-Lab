#include "../utils/unp.h"
#include "../utils/error.c"
#include "../utils/wrapsock.c"
#include<pthread.h>  // mulit-threading
#include<fcntl.h>

#define PATH "data/"

void *doRecv(void *sockfd){
	int fd=*((int *)sockfd);
	while(1){
		char buf[MAXLINE+1];

		int n=Read(fd,buf,MAXLINE);
		buf[n]=0;
		printf("----->\n%s\r\n",buf);

	}

}

void waitRandom(int time){
  int ums=rand()%time;
  int sec=rand()%2;
  struct timespec _t={sec,ums};
  usleep(ums);

}

int sendRandom(int limit){
  int rnd=rand()%limit+1;
  return rnd;

}



int  pollServer(int fd){
  int n;
  int r=sendRandom(10000);
  fprintf(stderr, "Willing to produce %d\n",r);
  n=send(fd,&r,sizeof(r),0);
  int reply;
  n=recv(fd,&reply,sizeof(reply),0);
  fprintf(stderr,"Successfully sent %d\n",r);
  return reply;

}
int main(int argc, char const *argv[])
{
   int sockfd,n;
   struct sockaddr_in servaddr;

   if(argc!=2){
   	err_quit("Usage ./client <PORT>");
   }

   sockfd=Socket(AF_INET,SOCK_STREAM,0);



   bzero(&servaddr,sizeof(servaddr));
   servaddr.sin_family=AF_INET;
   servaddr.sin_port=htons(atoi(argv[1]));

   servaddr.sin_addr.s_addr=INADDR_ANY; 
   // connect to any ipaddr

   // if(inet_pton(AF_INET,&argv[1],&servaddr.sin_addr)<=0)
		// err_quit("inet_pton error for %s",argv[1]);

   Connect(sockfd,(struct sockaddr*)&servaddr,sizeof(servaddr));

   fprintf(stderr,"---Connection Established---\n'");
 //  pthread_t thread;
  // pthread_create(&thread,0,doRecv,(void*)&sockfd);
   send(sockfd,"producer",sizeof("producer"),0);


   do{
	waitRandom(1231312); 
    int num=sendRandom(1132313),resp,req=1;
    int timeout=100;
    fprintf(stderr, "Selected number ---------------------------->%d\n",num);

    while(timeout<=1e6){
	usleep(11111);
      fprintf(stderr, "Trying to send request with timeout %dmus\n",timeout);
      req=1;
      n=send(sockfd,&req,sizeof(req),0);

      struct timeval tv;
      timeout=rand()%110011+timeout;  //exponential backoff for timeout 
      tv.tv_sec=0;
      tv.tv_usec=timeout;
      setsockopt(sockfd,SOL_SOCKET,SO_RCVTIMEO,(const char*)&tv,sizeof(tv));  //make server blocking 
      resp=0;
      n=recv(sockfd,&resp,sizeof(resp),0);

      if(n==-1&&resp!=1)continue;
	tv.tv_usec=0;
	
      setsockopt(sockfd,SOL_SOCKET,SO_RCVTIMEO,(const char*)&tv,sizeof(tv));



      n=send(sockfd,&num,sizeof(num),0);
      fprintf(stderr, "--------------------> Sent the number %d\n",num );
      break;
    }

   }while(1);
	return 0;
}
